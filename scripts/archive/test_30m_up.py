...
def main():
    print("="*60)
    print("🎯 Deep Ensemble 30m UP 모델 백테스트")
    print("📅 " + datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    print("="*60)

    # Load model
    try:
        model_data = joblib.load("models/deep_ensemble_30m_up_model.pkl")
    ...
    # Simple simulation (UP model always predicts UP)
    correct = up_count  # UP 모델이 항상 UP 예측시 맞는 횟수
    ...
    # Trading simulation with confidence
    np.random.seed(42)
    simulated_trades = 100
    win_rate = accuracy / 100  # Use model accuracy
